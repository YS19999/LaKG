...
def parse_args():
    ...
    # my settings
    parser.add_argument("--adaptive", type=int, default=20)
    parser.add_argument("--innersteps", type=int, default=1)
    parser.add_argument("--batchsize", type=int, default=1)
    parser.add_argument("--adaptive_lr", type=float, default=1e-1)
    parser.add_argument("--aba", type=bool, default=False)

    parser.add_argument("--threshold", type=float, default=0.5, help="threshold")

    parser.add_argument("--classifier", type=str, default="dckpn",
                        help="classifier. Options: [maml, tm, mmn, mpn, r2d2, dckpn]")
    parser.add_argument("--use_label", type=bool, default=False,
                        help="using label name")

    parser.add_argument("--bert", default=True, action="store_true")
    parser.add_argument("--pretrained", default="../module/bert-base-chinese", type=str,
                        help="path to the pre-trained bert embeddings.")

    # data configuration
    parser.add_argument("--dataset", type=str, default="jwoz_sh",
                        help="name of the dataset. "
                        "Options: [it, ac, at, fo, tr, sh, sc, na, we]")
    parser.add_argument("--shot", type=int, default=1,
                        help="#support examples for each class for each task")

    # maml configuration
    parser.add_argument("--maml_innersteps", type=int, default=10)
    parser.add_argument("--maml_batchsize", type=int, default=3)
    parser.add_argument("--maml_stepsize", type=float, default=1e-1)
    parser.add_argument("--maml_firstorder", action="store_true", default=True,
                        help="truncate higher order gradient")

    parser.add_argument("--n_train_class", type=int, default=50,
                        help="number of meta-train classes")
    parser.add_argument("--n_val_class", type=int, default=50,
                        help="number of meta-val classes")
    parser.add_argument("--n_test_class", type=int, default=50,
    ...
    parser.add_argument("--word_vector", type=str, default='sgns.wiki.char',
    ...
    parser.add_argument("--ebd_lr", type=float, default=1e-3, help="learning rate")
    parser.add_argument("--clf_lr", type=float, default=1e-3, help="learning rate")
    ...
    parser.add_argument("--notqdm", action="store_true", default=False,
                        help="disable tqdm")
    parser.add_argument("--result_path", type=str, default="results_dckpn_woz.json")
...
def main(shot=None, r=None, seed=None):
    """
    整体结构：
        1.main
        2.loader -> load_dataset
        3.mode is train or finetune -> factory - train
        4.mode is finetune or not -> factory - test
        5.saving results
    """
    args = parse_args()
    if r is not None:
    ...
    # load data
    train_data, val_data, test_data = loader.load_dataset(args)

    # initialize model
    model = {}
    model["clf"] = clf.get_classifier(args)

    if args.mode == "train":
        # train model on train_data, early stopping based on val_data
        train_utils.train(train_data, val_data, model, args)
    # else:
    #     model['clf'].load_state_dict(torch.load('tmp-runs/dckpn_cwoz_at_1/1.clf'))

    test_acc, test_std, test_p, test_pstd, test_re, test_restd, test_f1, test_f1std = train_utils.test(test_data, model, args)
    ...
    if args.result_path:
        ...
        result = {
            ...
            "test_std": test_std,
            "test_f1": test_f1,
            "test_f1std": test_f1std,
        }

        name = ['classifier', 'shot', 'dataset', 'lr', 'mode', 'seed']
