...
def train(train_data, val_data, model, args):
    ...
    # shutil.rmtree("tmp-runs/"+args.classifier + "_" + args.dataset + "_" + str(args.shot))
...
def train_one(task, model, opt, args, grad):
    ...
    support, query, label, id2label = task["support"], task["query"], task["label"], task["id2label"]

    # Apply the classifier
    acc, precision, recall, f1, loss, cla_loss = model['clf'](support=support, query=query, label=label, id2label=id2label)
    # file1.writelines(str(cla_loss) + "\n")
...
def test_one(task, model, args):
    ...
    if args.dataset in ['it', 'ac', 'at', 'fo', 'tr', 'sh']:
        opt = torch.optim.SGD(grad_param(model, ['clf']), lr=args.lr)
        adaptive = 1
    else:
        opt = torch.optim.SGD(grad_param(model, ['clf']), lr=1e-1)
        adaptive = 20

    for i in range(adaptive):
        
        pred, discriminative_loss = model['clf'](support=support, label=label, id2label=id2label)
        loss = F.multilabel_soft_margin_loss(pred, YS) + discriminative_loss
    ...
    # Apply the classifier
    pred, acc, precision, recall, f1 = model['clf'].metric(query=query, label=label, id2label=id2label)
